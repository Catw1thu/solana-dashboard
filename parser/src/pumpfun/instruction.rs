...
fn parse_create_v2_accounts(input: &InstructionInputRef<'_>) -> Option<CreateAccounts> {
    Some(CreateAccounts {
        mint: account_at(input, 0)?,
        mint_authority: account_at(input, 1).unwrap_or_default(),
        bonding_curve: account_at(input, 2).unwrap_or_default(),
        associated_bonding_curve: account_at(input, 3).unwrap_or_default(),
        global: account_at(input, 4).unwrap_or_default(),
        user: account_at(input, 5).unwrap_or_default(),
        system_program: account_at(input, 6).unwrap_or_default(),
        token_program: account_at(input, 7).unwrap_or_default(),
        associated_token_program: account_at(input, 8).unwrap_or_default(),
        event_authority: account_at(input, 14).unwrap_or_default(),
        program: account_at(input, 15).unwrap_or_default(),
